export const promptOnDelete = (vue, description = '提示', callback = {}) => {
...
export const promptOnAction = (vue, description = '确认操作', callback = {}) => {
  if (description === null) description = '确认操作'
  vue.$prompt(`${description}，请输入OK`, '确认操作', {
    inputErrorMessage: '输入不正确',
    inputValidator(value) {
      if (value === null) return false
      if (value.toLowerCase() === 'ok') return true
      return false
    }
  }).then((value) => {
    callback(value)
...
export const confirmation = (vue, description = '确认操作，是否继续？', callback = {}) => {
